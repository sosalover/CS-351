...
var VSHADER_SOURCE = 
  ...
  '  myWave = u_shiver.x * sin(1.0 * gl_Position.y + u_shiver.y); \n' +
  '  gl_Position.x += myWave; \n' +
  '  gl_Position.y += myWave; \n' +
  '  gl_PointSize = 10.0;\n' +
  '  v_Color = a_Color;\n' +
  '}\n';
...
var shiverY = 0.0;
var shiverYStep = 1.0;													// (x,y,z,w)position + (r,g,b)color
...
//=====================================================================
  ...
  if (!u_shiver) { 
    console.log('Failed to get the storage location of u_shiver');
    return;
  }
...
//-----------------  
  ...
  var tick = function() {
	...
	shiverY = animateShiver(shiverY);
	drawAll(gl, n, currentAngle, modelMatrix, u_ModelMatrix, u_shiver);   // Draw shapes
...
function drawAll(gl, n, currentAngle, modelMatrix, u_ModelMatrix, u_shiver) {
//==============================================================================

// Clear <canvas>  colors AND the depth buffer
  gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
  modelMatrix.setIdentity();    // DEFINE 'world-space' coords.
  var shiver = [-0.5,shiverY];
  gl.uniform2fv(u_shiver, shiver);
/*
...
function animateShiver(oldShiver){
	var now = Date.now();
	var elapsed = now - g_last1;
	g_last1 = now;

	
	if (oldShiver > 2.0 * Math.PI){
		shiverYStep = -1.0;
	}else if (oldShiver < 0){
		shiverYStep = 1.0;
	}
	var newShiver = oldShiver + (shiverYStep * elapsed)/1000.0;
	return newShiver;
}
